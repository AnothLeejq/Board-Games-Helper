function createCharacter(name, outName, explaination, side) {
    return {
        name: name,
        outName: outName,
...
function showExplaination(name) {
    var element = spCharacters[0];
...
function findSpCharacter(query) {
    var queryKey, queryKeyword, target;
    target = query["target"];
    if ("name" in query) {
        queryKey = "name";
...
window.onload = function () {
    ...
    for (var i = 0; spCharacters[i]; ++i) {
        spcsRefHTML += "<button onclick=\"showExplaination('".concat(spCharacters[i].name, "')\">What is ").concat(spCharacters[i].outName, "</button>         ");
    }
